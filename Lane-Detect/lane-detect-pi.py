...
class Lines():
    ...
    def calculate_curvature_offset(self):
        ...
        if self.detected_first:
            ...
            ym_per_pix = 2.0/250 #50/250 # meters per pixel in y dimension
            xm_per_pix = 0.5/75 #3.7/75 # meters per pixel in x dimension
    ...
    def project_on_road_debug(self, image_input):
        ...
        if self.detected_first & self.detected:
            ...
            # SEND SERIAL DATA TO CYPRESS MICROCONTROLLER
            #~ dataToSend = [1,2,3,4,5,6,7,8]
            #~ print("Enviando: ", dataToSend)
            #~ ser.write(dataToSend)
            #~ time.sleep(0.25)
            #~ dataRead = ser.read(8)
            #~ print("Se leyó: ", dataRead)
